...
# 1) Input
job_name = "qa_bru_stg_patients"
source_table = 'bru_stg.stg_patients'
test_rules = [
        # original column, new column, rule
        ("id", "check_id_isnull", "id IS NOT NULL OR id != '' OR lower(id) != 'null'")
]
...
# check id count - no duplication in id col
number_of_id =  df.select('id').count()
number_of_unique_id = df.select('id').distinct().count()

if number_of_id != number_of_unique_id:
    print('--- Customized Rules Failed - id uniqueness ---')
